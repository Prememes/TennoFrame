...
def _get_damage(w):
    """Always returns a number for weapon damage, even when the API returns a dict."""
    d = w.get("totalDamage") or w.get("damage")
    if isinstance(d, dict):
        return round(sum(v for v in d.values() if isinstance(v, (int, float))), 1)
    return d
...
if os.path.exists(html_path):
    ...
    open_tag  = '<script id="inline-data">'
    close_tag = '</script>'
    repl_str  = open_tag + '\n' + data_iife + '\n' + close_tag
    new_html, n = re.subn(pattern, lambda _: repl_str, html, flags=re.DOTALL)
